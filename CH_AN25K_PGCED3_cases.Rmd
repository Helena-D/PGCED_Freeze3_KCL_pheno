...
barcodes <- read_csv("/Users/helenadavies/King's College London/MT-Translational Neuropsychiatric Genomics - Helena_Davies_PhD - Helena_Davies_PhD/PGC_Freeze3_cases/data_raw/charlottes_helix/barcodes.csv")
...
Check visit notes and barcode
```{r check visit notes and barcodes}
dat_raw %>%
  select(contains("visit_barcode")) %>%
  colnames()

dat_raw %>%
  select(contains("visit_notes")) 
...
dat <- dat_raw %>% 
  select(
    ...
    "visit_notes_10_x",
    "visit_notes_11_x",
     "registration_notes_x",
    "No Known Diagnosis_x",
    "Anorexia Nervosa_x",
    "Bulimia Nervosa_x",
    "ED-NOS_x",
    "Binge Eating_x",
    ...
    "core_secdiag_x",
    "core_bmi_x",
    "registration_notes_y",
    "No Known Diagnosis_y",
    "Anorexia Nervosa_y",
    "Bulimia Nervosa_y",
    "ED-NOS_y",
    "Binge Eating_y",
...
dat %>%
  colnames()
```
# Merge dat with barcode data
```{r merge data with barcode data}
dat <- dplyr::full_join(dat,
                        barcodes,
                        by ="ID")

dat %>%
  freq(visit_barcode_x)
```
...
missing_dat <- dat %>%
  filter(
    ...
    is.na(visit_notes.1_x) &
    is.na(visit_notes_2.1_x) &
    is.na(visit_notes_3.1_x) &
    is.na(visit_notes_4_x) &
    is.na(visit_notes_5_x) &
    is.na(visit_notes_6_x) &
    is.na(visit_notes_7_x) &
    is.na(visit_notes_8_x) &
    is.na(visit_notes_9_x) &
    is.na(visit_notes_10_x) &
    is.na(visit_notes_11_x) &
...
```
146 people with all missing data.
...
# Check
dat %>%
  freq(NUMBER_freq_binge_per_week)
...
# Change to numeric
dat$NUMBER_freq_binge_per_week_numeric <- as.numeric(dat$NUMBER_freq_binge_per_week)

# Check again
dat %>%
  freq(NUMBER_freq_binge_per_week_numeric)
```
  ...
  mutate(binge_eating_loss_control_DSM5 =
           case_when(
             ...
             (out_of_control_bingeing == "Yes, definitely out of control" |
               ...
               (NUMBER_freq_binge_per_week_numeric >= 1 &
...
```
206 people with DSM-5 binge eating.
...
dat %>%
  freq(diagnosis_BN)
```
12 people with a diagnosis of BN from clinician.

3. Has a self-reported diagnosis of BN or BED, with specification that this has been confirmed by a healthcare professional

For Charlotte's Helix, the variables 'Bulimia Nervosa_x' and 'Bulimia Nervosa_y' *could* include diagnoses but could also include *not diagnosed* individuals..
...
dat %>%
  freq(reg_info_BN)
```
2 people with clinically-confirmed diagnosis of BN.

Visit notes
NB: visit_notes_x, visit_notes_2_x, visit_notes_3_x are the same as visit_notes.1_x, visit_notes_2.1_x, and visit_notes_3.1_x
All empty: visit_notes_5_x, visit_notes_6_x,visit_notes_7_x,  visit_notes_8_x, visit_notes_9_x, visit_notes_10_x, visit_notes_11_x
```{r visit notes narrow BE}
dat %>%
  freq(visit_notes_x)
# Case: Binge eating disorder; Case: Bulimia Nervosa; CAse: Bulimia Nervosa; Case: previously anorexia nervosa/ Bulimia; DX: Bulimia Nervosa (these all count as BE_broad not BE_narrow)

dat %>%
  freq(visit_notes_2_x)
# Case: Bulimia Nervosa; Case: Bulimia Nervosa, Coeliac, BPD, Anaemia;  (these all count as BE_broad not BE_narrow)

dat %>%
  freq(visit_notes_3_x) # None relevant

dat %>%
  freq(visit_notes_4_x) # None relevant
```
...
```
353 people with broad binge eating (i.e., without freq and dur criteria)
...
```
21 people have a diagnosis of bulimia or binge-eating disorder recorded in the reg notes.

Visit notes
NB: visit_notes_x, visit_notes_2_x, visit_notes_3_x are the same as visit_notes.1_x, visit_notes_2.1_x, and visit_notes_3.1_x
All empty: visit_notes_5_x, visit_notes_6_x,visit_notes_7_x,  visit_notes_8_x, visit_notes_9_x, visit_notes_10_x, visit_notes_11_x
```{r visit notes broad BE}
dat %>%
  freq(visit_notes_x)
# Case: Binge eating disorder; Case: Bulimia Nervosa; CAse: Bulimia Nervosa; Case: previously anorexia nervosa/ Bulimia; DX: Bulimia Nervosa 

dat %>%
  freq(visit_notes_2_x)
# Case: Bulimia Nervosa; Case: Bulimia Nervosa, Coeliac, BPD, Anaemia 
...
dat <- dat %>%
  mutate(visit_notes_BE_broad_1 =
           case_when(
             visit_notes_x == "Case: Binge eating disorder" |
               visit_notes_x == "Case: Bulimia Nervosa" |
               visit_notes_x == "CAse: Bulimia Nervosa" |
               visit_notes_x == "Case: previously anorexia nervosa/ Bulimia" |
               visit_notes_x == "DX: Bulimia Nervosa"~ 1
           ))

# Visit notes 2
dat <- dat %>%
  mutate(visit_notes_BE_broad_2 =
           case_when(
             visit_notes_2_x == "Case: Bulimia Nervosa" |
               visit_notes_2_x == "Case: Bulimia Nervosa, Coeliac, BPD, Anaemia" ~ 1
           ))

# create single variable for visit notes
dat <- dat %>%
  mutate(visit_notes_BE_broad_final =
           case_when(
             visit_notes_BE_broad_1 == 1 |
             visit_notes_BE_broad_2 == 1  ~ 1
             
           ))

# Check
dat %>%
  freq(visit_notes_BE_broad_final)
```
15 people are identified BE_broad via visit notes.
...
visit_notes_4_x does not contain any relevant information.
```{r visit notes AN}
# Visit notes 1
dat <- dat %>%
  mutate(visit_notes_AN_1 =
           case_when(
             visit_notes_x == "AN" |
               ...
               visit_notes_x == "Case: Anorexia (Recovered)" |
               ...
               visit_notes_x == "Case: previously anorexia nervosa/ Bulimia" | 
               visit_notes_x == "Gave blood prior to AN Dx" |
...
dat <- dat %>%
  mutate(visit_notes_AN_3 =
           case_when(
             visit_notes_3_x == "AN" |
               visit_notes_3_x == "AN  Samples taken using Oxytocin study consent form  as participant has been BioResourced previously"  ~ 1
           ))
...
```
55 people meet AN criteria in the visit notes.
...
# Look at min and max
descr(dat$AN_BMI) # All values are between 8 and 38 so are all plausible.
```
...
```
383 people meet criteria for ANBP via this algorithm.
   ...
   (binge_eating == "No" | # Not ever binged
    binge_eating_only_low_weight == "No")  & # Binged but not at low weight
     ...
     #...or purging behavior (i.e., self-induced vomiting or the misuse of laxatives, diuretics, or enemas)...  
        ...
        # NB: Response for `selfinduced_vomiting_at_low weight`, for example, are "Making yourself vomit" or NA. Wouldn't normally assume that a value of missing is indicative of a lack of endorsement, but in this case (for the purging behaviours at low weight) there doesn't seem to be another option. 
     (
...
```
223 people meet criteria for ANR via this algorithm.
...
```
611 meet criteria for AN via this algorithm.
...
```
84 people meet criteria for AN in the reg notes.
...
```
43 people meet criteria for AN in the primary or secondary diagnosis.
...
```
2 people meet ANBP criteria in reg notes.
...
```
3 people meet ANR criteria in reg notes.

## 6. Recruitment into 100,000 Genomes Project 
'BED' These people were recruited based on having severe and enduring AN with a BMI of below 16. We can assume that they have a diagnosis of AN.
...
dat <- dat %>%
  mutate(AN_100K =
           case_when(  visit_barcode_x == "BED00567" |
                       ...
                       # Already identified all people (n=53) so do not need to also add all the other barcodes
           ))

dat %>%
  freq(AN_100K)
```
53 people were recruited in 100k Genomes Project.
...
dat <- dat %>%
  mutate(BOA_AN =
           case_when(
             str_detect(visit_barcode_3_x, "BOA") |
             str_detect(visit_barcode_2_x, "BOA") |
             str_detect(visit_barcode_x, "BOA") |
               
             str_detect(visit_barcode.1_x, "BOA") |
             str_detect(visit_barcode_2.1_x, "BOA") |
             str_detect(visit_barcode_3.1_x, "BOA") |
               
             str_detect(visit_barcode_4_x, "BOA") |
             str_detect(visit_barcode_5_x, "BOA") |
             str_detect(visit_barcode_6_x, "BOA") |
               
             str_detect(visit_barcode_7_x, "BOA") |
             str_detect(visit_barcode_8_x, "BOA") |
             str_detect(visit_barcode_9_x, "BOA") |
               
             str_detect(visit_barcode_10_x, "BOA") |
             str_detect(visit_barcode_11_x, "BOA") |
             str_detect(visit_barcode_9_x, "BOA") ~ 1
             )
  )
...
dat %>%
  freq(BOA_AN)
```
51 people meet criteria for AN via being a BOA sample.
...
dat %>%
  freq(AN_case)
```
700 people are an AN case for PGC.
...
dat <- dat %>%
  mutate(ANR_case =
           case_when(DSM5_AN_restricting_binary_numeric == 1 |
                      ANR_registration == 1 & 
                       (ANBP_registration != 1 &
                        DSM5_AN_binge_purge_binary_numeric != 1)  ~ 1 # ANBP > ANR (as soon as you have binged/purged during low weight, you are ANBP)
                     )
         )
...
# Bind with control data from NBR
```{r bind with NBR controls}
# Read in control data
CH_controls <- readRDS(file = "/Users/helenadavies/King's College London/MT-Translational Neuropsychiatric Genomics - Helena_Davies_PhD - Helena_Davies_PhD/PGC_Freeze3_cases/data_cleaned/Charlottes_Helix_controls_280322.rds") 

# Convert IDs to character to enable bind
dat.final.no.dup$ID <- as.character(dat.final.no.dup$ID)
  
# Bind data
dat_final <- dat.final.no.dup %>%
  bind_rows(CH_controls)

dat_final %>%
  colnames()

dat_final %>% # Check = 1700
  freq(control_final)
```

# Save case and control data
```{r Save dataset}
saveRDS(object = dat_final, file = paste0("/Users/helenadavies/King's College London/MT-Translational Neuropsychiatric Genomics - Helena_Davies_PhD - Helena_Davies_PhD/PGC_Freeze3_cases/data_cleaned/data_CharlottesHelix_cleaned_case_control", date, ".rds"))
```
