...
3. Has a self-reported diagnosis of BN or BED, with specification that this has been confirmed by a healthcare professional; Note that any cases of ANB(P) would be captured by the binge eating variable. Inclusion of BN / BED also includes self-report cases. (we do not include an ANBP self-report option but even if we did, it doesn't necessarily include binge eating as could be purging only)
...
# ANOREXIA PHENOTYPES
We are doing 5 GWASs; ANBP, ANR, AN, BE_broad, BE_narrow
...
Weight before weight loss (COPING GLAD & NBR), i.e., START weight
...
Work out weight at period of significant weight loss (COPING GLAD & NBR), i.e., weight AFTER weight loss
...
## AAN cases
### BMI of 18.5 or less [GLAD] - These will be counted as cases
...
dat_with_selfreport <- dat_with_selfreport %>%
    mutate(
      AAN_LOWBMI_GLAD =
        ...
        (  
          (
            ed.DSM5_AN_atypical_low_BMI_binary_numeric == 1 
          )
        ),
      true = 1,
          false = 0,
...
### BMI of 18.5 or less [COPING GLAD and NBR] - these will be counted as cases
...
### Cross-check BMI with AN/AAN status
#### GLAD
Adding this chunk to double-check that those who answered the AAN module but had a BMI below 18.5 are counted as AN cases.
```{r cross-checking BMI with ED status GLAD}
dat_with_selfreport <- dat_with_selfreport %>%
  mutate(
    BMI_AAN_module_below_18.55_numeric =
      if_else(
        condition = AAN_BMI <= 18.55,
        true = 0,
        false = 1,
        missing = NA_real_
        )
...
dat_with_selfreport <- dat_with_selfreport %>%
  mutate(
    BMI_AAN_module_below_18.55 =
      recode_factor(
        BMI_AAN_module_below_18.55_numeric,
        `0` = "BMI lower than 18.55 kg/m2",
        `1` = "BMI higher than 18.55 kg/m2"
        )
    )

dat_with_selfreport %>%
  freq(BMI_AAN_module_below_18.55,
    cumul = FALSE,
    round.digits = 1
    )
...
# Cross-check
dat_with_selfreport %>%
  group_by(BMI_AAN_module_below_18.55) %>%
  count(AAN_LOWBMI_GLAD)
```

### Cross-check BMI with AN/AAN status
#### GLAD COPING/NBR
Adding this chunk to double-check that those who answered the AAN module but had a BMI below 18.5 are counted as AN cases.
```{r cross-checking BMI with ED status GLAD COPING/NBR}
dat_with_selfreport <- dat_with_selfreport %>%
  mutate(
    BMI_AAN_module_below_18.55_numeric_cop =
      if_else(
        condition = AAN_BMI_cop <= 18.55,
        true = 0,
        false = 1,
        missing = NA_real_
        )
...
dat_with_selfreport <- dat_with_selfreport %>%
  mutate(
    BMI_AAN_module_below_18.55_cop =
      recode_factor(
        BMI_AAN_module_below_18.55_numeric_cop,
        `0` = "BMI lower than 18.55 kg/m2",
        `1` = "BMI higher than 18.55 kg/m2"
...
dat_with_selfreport %>%
  freq(BMI_AAN_module_below_18.55_cop,
    cumul = FALSE,
    round.digits = 1
    )
...
# Cross-check
dat_with_selfreport %>%
  group_by(BMI_AAN_module_below_18.55_cop) %>%
  count(AAN_LOWBMI_COPING)
```
...
The below are people who have atypical anorexia.

1. Answered the AN module but have a BMI ABOVE 18.5 
2. Answered the AAN module and have a BMI above 18.5 

## Atypical anorexia nervosa module with BMI over 18.55 [COPING]
...
## Second, need to consider IBD/IBS cases
...
dat_nbr_health.id <- dat_nbr_health  %>% 
  ...
  add_column(sample = "NBR", 
             .before = "health.inflammatory_bowel_disease_crohns") %>% #create new column 
   select(
         ID = subjectid,# ID
         sample,
...
dat_nbr_AN_screener.id <- dat_nbr_AN_screener  %>% 
  ...
  remove_duplicates("subjectid")  %>%
  add_column(sample = "NBR", 
             .before = "an.weigh_weighed_people_thought") %>% #create new column 
   select(
         ID = subjectid,# ID
         sample,
...
## Need to limit to those with genotype data
```{r NBR with geno data}
...
# for now
NBR_geno <- NBR_geno %>%
  select(ID = "5aae667ce865dbc91d74b90ad3df")

dat_nbr_AN_screener.id <- dat_nbr_AN_screener.id %>%
  filter(ID %in% NBR_geno$ID)
```
           ...
           case_when(
             mhd.suspected_eating_disorder_diagnosed_cop_numeric == 0 | # Never suspected an eating disorder
                 ...
                 (mhd.suspected_eating_disorder_diagnosed_cop_numeric == -888 & # Not sure if suspected eating disorder...
                   mhd.none_eds_cop_numeric == 1) #...but never been diagnosed with an eating disorder
                  ~ 1 
                       )
         )
...
(469/7025)*100 # 6.67% Will leave as is
...
(720/7025)*100 # 10.25% - Need to randomly select 92 people to keep (i.e., 1.3% of 7025)
(138/10598)*100 # 1.3%
...
No_IBD_controls <- dat_with_selfreport %>% 
  filter(IBD == 0 &
         control == 1)  %>%
  pull(ID)
...
dat_with_selfreport %>%
  freq(control_final) # 6368
```

# Set aside 1400 controls for Charlotte's Helix
```{r 1400 controls for Charlottes Helix}
# Random selection
set.seed(123)

CH_controls <- dat_with_selfreport %>%
  filter(control_final == 1) %>%
  sample_n(1400) 

# Save dataset
CH_controls %>%
  saveRDS(file = "/Users/helenadavies/King's College London/MT-Translational Neuropsychiatric Genomics - Helena_Davies_PhD - Helena_Davies_PhD/PGC_Freeze3_cases/data_cleaned/Charlottes_Helix_controls_090322.rds") 

# Create new GLAD/NBR dataset without controls used in Charlotte's Helix
`%!in%` = Negate(`%in%`)
...
dat_with_selfreport <- dat_with_selfreport %>%
  mutate(control_final =
           case_when(
             control_final == 1 &
             ID %!in% CH_controls$ID ~ 1, # Set to being a control 
             
             control_final == 1 &
             ID %in% CH_controls$ID ~ 0, # No longer a control (in this dataset)
                        )
         )

# Check (should now be 1400 fewer controls than previously)
dat_with_selfreport %>%
  freq(control_final)
```
...
# 71 people who met broad BE criteria in GLAD but didn't meet narrow BE criteria, but then went on to meet it in COPING. Therefore need to also split narrow and broad BE when looking at assigning additional variables.
```
...
dat %>%
  select(ID,
       ...
       BE_broad,
       control) %>%
saveRDS(file = "/Users/helenadavies/King's College London/MT-Translational Neuropsychiatric Genomics - Helena_Davies_PhD - Helena_Davies_PhD/PGC_Freeze3_cases/data_cleaned/PGC_GLAD_NBR_case_control_all_pheno_070322.rds")
