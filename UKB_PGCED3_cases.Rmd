...
# Check same as nrow
length(unique(hesin_merged_ID$eid)) # 123
```
...
hesin_neo_ID %>%
  count(neoplasms_ICD9) # 176

hesin_neo_ID %>%
  count(no_neoplasms_ICD9) # 20281
```
...
hesin_diag_exclusion_ICD10_ID %>%
  count(ICD10_malignant_neo) # 22613
```
...
hesin_exclusion_dat %>%
  count(HES_exclusion_criteria) # 69823
...
hesin_exclusion_dat %>%
  count(HES_data_entry) # 435465 - Most people have at least one HES data entry (only 68 people do not have any HES data entries)
```
...
length(unique(death_data_cases$eid))
# Check
...
# Merge data up until this point and save
```{r merge data up until this point and save}
dat_interim_to_save <- list(
  death_data_cases,
...
```{r save dataset}
saveRDS(object = dat_interim_to_save, paste0("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED3_MHQ_HES_DEATH_case_data.rds"))
```
...
For the below codes, we need more information to make a decision as to whether to include them as 'anorexia nervosa' cases. This is because a single code is linked to multiple phenotypes, some of which would be considered cases but others that would not be. Specifically, these codes include a phenotype which does not even mention the word 'anorexia' but references 'loss of appetite'
...
# XM07X = Lack of appetite, Anorexia, Off food, No appetite, Anorexic 
gp_clinical <- gp_clinical %>%
...
gp_clinical %>%
  count(read_2_bulimia_or_compulsive_eating_disorder) # 2 
```
...
## E2751 = 'Compulsive eating disorder (& [bulimia including non-organic overeating])' OR 'Compulsive eating disorder' OR 'Bulimia (non-organic overeating)' +++ not sure these count yet
#gp_clinical <- gp_clinical %>%
...
GP_dat <- list(
 ...
 gp_anorexia_read2_need_excl_criteria_cross_val,
 gp_anorexia_read3_need_excl_criteria_cross_val,
 gp_anorexia_read2_3_eating_disorder_clinic,
 gp_BE_broad,
 gp_BE_narrow
  ) %>% 
  reduce(full_join, 
...
rm(gp_anorexia_read2_3_eating_disorder_clinic)
rm(gp_BE_broad)
rm(gp_BE_narrow)
gc()
```
...
covar_dat <- readRDS(file = "/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED3_covariates.rds")
...
covar_PCs <- read.table("/mnt/lustre/groups/ukbiobank/Edinburgh_Data/usr/lee/UKB_PGC_ED/EUR_FLASHPCA/pcs.txt") # ~40 PCs, we only want up to PC 6
...
Read in batch data
```{r read in PC data from Lee}
covar_batch <- read.table("/scratch/groups/ukbiobank/ukb18177_glanville/genotyped/ukb18177_glanville_binary_pre_qc.fam") # text file. Can cross-check sex information with covariate file to check for errors!
...
```{r merge the two files PC and batch}
...
```{r save covar data with batch and PC info}
write_tsv(covar_dat_batch_PC, file = "/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED3_covariates_merged_PC_batch.txt")
```

# Edit: 15/2/22
regenie covariate files need to have no missing data.
```{r remove missing data for regenie}
covar_dat <- read_tsv("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED3_covariates_merged_PC_batch.txt")

covar_dat_no_na <- covar_dat[complete.cases(covar_dat), ]
```

```{r save covar data with batch and PC info no NAs}
write_tsv(covar_dat_no_na, file = "/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED3_covariates_merged_PC_batch_no_NA.txt")
```
