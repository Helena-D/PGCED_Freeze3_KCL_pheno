...
# R0360 = '[D]Bulimia NOS' OR '[D]Excessive eating' [neither case nor control]
gp_clinical <- gp_clinical %>%
...
# E2751 = Bulimia (non-organic overeating) OR Compulsive eating disorder [neither case nor control]
gp_clinical <- gp_clinical %>%
...
gp_BE_broad <- gp_clinical %>%
  filter(read_2_Xbulimia_nervosa_OR_bulimiaNOS == 1 |
                   # read_2_bulimia_nonorganic_overeating == 1 | # Not a case as includes 'Compulsive eating disorder'
...
gp_BE_narrow <- gp_clinical %>%
  filter(read_2_Xbulimia_nervosa_OR_bulimiaNOS == 1 |
                   # read_2_bulimia_nonorganic_overeating == 1 | # Not a case as includes 'Compuslive eating disorder'
                    read_3_suspected_BED == 1 |
                    read_3_BN == 1 
           ) 
...
```{r}
str(batch_dat_final$IID)

str(PCs_dat_final$IID)

head(batch_dat_final)
head(PCs_dat_final)
```
