...
saveRDS(object = dat_interim, file = paste0("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/interim_UKB_PGCED3_case_data.rds"))
```
...
gp_clinical %>%
  count(read3_seen_in_eating_disorder_clinic)  # 7
```
...
```{r save GP AN data} 
saveRDS(object = GP_dat_AN, file = paste0("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_GP_AN_data.rds"))
```
...
gp_BE_broad <- gp_clinical %>%
  filter(read2_binge_eating == 1 |
         ...
         read_2_bulimia_or_compuslive_eating_disorder == 1 |
...
Save data so far
```{r save data}
saveRDS(object = dat, paste0("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/interim2_UKB_PGCED3_case_data.rds"))
```
...
# Check how many pass cross-validation
dat %>%
...
### Identify those of the 260 with diabetes
...
saveRDS(object = all_variables_case_dat,
        file = paste0("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_all_case_data_230222.rds")) # NB: Have a separate .rmd for file formatting for Plink and regenie
```
...
saveRDS(object = only_case_dat, file = paste0("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_only_case_data.rds")) # NB: Have a separate .rmd for file formatting for Plink and regenie
```
...
saveRDS(object = covariates, file = paste0("/scratch/groups/ukbiobank/usr/UKB_PGCED_Freeze3_final_files/helena_d/UKB_PGCED3_covariates.rds"))
```

# Additional - PCs and batch (from Lee)
```{r read in covariate data}
covar_dat <- readRDS(file = "/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_covariates.rds")
...
write_tsv(covar_dat_batch_PC, file = "/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_covariates_merged_PC_batch.txt")
```

# Edit: 15/2/22
regenie covariate files need to have no missing data.
```{r remove missing data for regenie}
covar_dat <- read_tsv("/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_covariates_merged_PC_batch.txt")

covar_dat_no_na <- covar_dat[complete.cases(covar_dat), ]
```

```{r save covar data with batch and PC info no NAs}
write_tsv(covar_dat_no_na, file = "/scratch/groups/ukbiobank/usr/helena_d/UKB_PGCED_Freeze3_final_files/UKB_PGCED3_covariates_merged_PC_batch_no_NA.txt")
```
